...
- **A broadcast's audience is built from rules.** `send_broadcast` takes `{kind:"filter", match:"all"|"any", rules:[{field, op, value, not}]}` — standing, tier, plan, home city, league, held for dues, aboard or waitlisted on an episode, verified phone, in the show, in the directory, an upcoming pass, joined before/after a date, knots or nights above/below a figure — up to twelve rules, each negatable, resolved by `resolve_broadcast_audience` (no grant; the fan-out and the clock call it) and previewed by `broadcast_audience_preview` (staff). The five old kinds still resolve as rules. A word to nobody is refused before it is recorded.
