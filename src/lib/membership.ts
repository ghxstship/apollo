...
/* ── The member number ──────────────────────────────────────────────────────
   ...
   stripping a prefix it assumed was still stored, the other rendering the column
   raw — and the till could find a member by neither the card face nor the card.
   Both readings are now unnecessary: memberNumberTail matches the DIGITS, which
   are the same in every generation of the number, so nothing here depends on
   where the prefix lives or whether there is one.

   memberNumber() is kept for display: a stored value may still arrive with a
   prefix from a fixture, an import or a hand-written row, and the card face
   shows the bare number either way.
*/
