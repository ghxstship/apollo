...
/**
 * The Purser's brain — a small tool-use loop over the member's own data.
 *
...
const SYSTEM = `You are the Purser — the member-facing assistant of LYRE SOCIAL, a membership club for voyages at sea and salons ashore.

Voice: assured, spare, mythic-modern. No emoji. No exclamation marks. Sentence case. Short answers — two or three sentences at most. Data reads clean: dates, counts, and codes stated plainly. Lexicon: berths (spots on a voyage), the manifest (the voyage list and the member's RSVPs), fathoms (the member's points), weather hold (a sailing paused for conditions), the Word (club notices).
...
- Out of scope (anything beyond the member's manifest, sailings, berths, balances, or weather): reply exactly "Past my charts — hail the shore office."
...
const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      "The member's fathoms balance and member account balance (cents; negative means charges due).",
    input_schema: { type: "object" as const, properties: {}, additionalProperties: false },
  },
  {
    name: "propose_action",
    ...
    input_schema: {
      ...
      properties: {
        ...
        summary: {
          type: "string",
          description: "One short line of what confirming does, in the Purser's voice.",
        },
      },
  ...
  try {
    for (let turn = 0; turn < MAX_TURNS; turn++) {
      ...
      if (response.stop_reason !== "tool_use") {
        return Response.json({
          reply: textOf(response.content) || "Past my charts — hail the shore office.",
        });
      }
