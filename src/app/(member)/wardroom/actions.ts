...
export type WardroomResult = { error?: string };
...
export async function toggleHail(postId: string, hailed: boolean): Promise<WardroomResult> {
  const { supabase, userId } = await member();
  if (!userId) return { error: "Sign in first." };
  const { error } = hailed
    ? await supabase
...
export async function addComment(postId: string, body: string): Promise<WardroomResult> {
  const { supabase, userId } = await member();
  if (!userId) return { error: "Sign in first." };
...
export async function deletePost(postId: string): Promise<WardroomResult> {
  const { supabase, userId } = await member();
  if (!userId) return { error: "Sign in first." };
  const { error } = await supabase
    .from("wardroom_posts")
