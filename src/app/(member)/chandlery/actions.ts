...
export type ChandleryResult = { error?: string };
...
export async function placeShopOrder(lines: CrateLine[]): Promise<ChandleryResult> {
  ...
  revalidatePath("/chandlery");
  return {};
}

export async function requestRefund(orderId: string): Promise<ChandleryResult> {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
