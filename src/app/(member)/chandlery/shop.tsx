...
const ORDER_BADGE: Record<string, { tone: "brass" | "ink" | "laurel" | "clay" | "outline"; label: string }> = {
  placed: { tone: "outline", label: "Placed" },
  fulfilled: { tone: "laurel", label: "Fulfilled" },
  refund_requested: { tone: "clay", label: "Refund requested" },
  refunded: { tone: "ink", label: "Refunded" },
};

type CartLine = CrateLine & { name: string; priceCents: number };

export function ChandleryShop({
  products,
  isGlobal,
  orders,
}: {
        ...
        {shown.map((p) => (
          <button key={p.id} type="button" className="chd-tile" onClick={() => openProduct(p)}>
            <span className="chd-tile__media" style={{ background: CATEGORY_SEA[p.category] }}>
              {p.badge ? <Badge tone="brass">{p.badge}</Badge> : null}
            </span>
      ...
      <Dialog
        ...
        footer={
          <>
            ...
            <Button variant="brass" size="sm" onClick={addToCrate}>
              Add to the crate
            </Button>
          </>
        }
          ...
          <aside className="chd-drawer" role="dialog" aria-label="The crate">
            ...
            {cart.length > 0 ? (
              <div className="chd-drawer__foot">
                ...
                <Button variant="brass" fullWidth disabled={pending} onClick={checkout}>
