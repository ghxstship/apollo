...
import { fathoms, logDate, logMeta } from "@/lib/format";
      ...
      {live.length > 0 ? (
        <section className="mbr-sec ls-rise-2">
          ...
          <div className="hbr-live">
            {live.map((v) => (
              <Link key={v.id} href="/now" className="mbr-plain">
      ...
      <section className="mbr-sec ls-rise-2">
        <div className="ls-grid-2">
          <div style={{ border: "1px solid var(--line-faint)", background: "var(--surface-card)", padding: 24 }}>
            <Stat label="Fathoms" value={fathoms(balance)} sub="MILES, NOT LIKES" />
          </div>
          <div className="hbr-links" style={{ gridTemplateColumns: "1fr" }}>
            <Link href="/wardroom" className="hbr-link">
            ...
            <Link href="/portal" className="hbr-link">
              ...
              <div>
                <b>Member portal</b>
                <span>FATHOMS · REWARDS · REFERRALS</span>
              </div>
            </Link>
      ...
      <section className="mbr-sec ls-rise-3">
        ...
        {word.length === 0 ? (
          <StateBlock
            ...
            bare
            title="Quiet water."
            detail="Weather, berths, and fathoms land here."
          />
        ) : (
