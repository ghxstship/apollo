...
/* One motion for every ruling. decide_a_proposal is staff-checked inside,
   ...
   An approval may name the sailing it became. The link is written AFTER the
...
export async function decideProposal(
  ...
  if (ruling === "approved" && voyageId) {
    const { error: linkError } = await supabase
      ...
      .eq("id", id);
    if (linkError) {
      linkNote = "Approved and the proposer told, but the sailing did not link. Pick it again from the row.";
    }
  }
...
/* Link (or re-link) an approved proposal to the sailing it became. */
...
/* — Charter requests: an on-request format has a door —
   ...
   record is written first; if the Word cannot be sent, the operator is told
...
export async function decideCharter(
  ...
  const title =
    ruling === "answered" ? "The Bridge answered your charter request" : "The Bridge passed on your charter request";
  ...
  if (wordError) {
    return {
      note: "Ruled, but the Word did not reach them — tell them from Shoreside.",
    };
  }
  return {};
}
