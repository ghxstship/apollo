...
/* — The flotilla. voyage_vessels has had readers since the fleet landed
     ...
     assigned to this episode yet" with no way to assign one. These two are that
     policy's writer. — */
...
export async function createVoyage(input: NewVoyageInput): Promise<ActionResult> {
  ...
  /* An episode with no return time can never round the marks that are measured
     ...
     long-passage (8h aboard) and night-reckoning (still out at the city's
     midnight) were unreachable for every episode the Bridge has ever set. The
  ...
  /* The on-sale hour reads on the same city clock as the departure. Blank
...
export type VoyageProgramInput = {
  ...
  seasonId: string | null;
  venueId: string | null;
  /* Wall clock on the episode's city, or null = on sale immediately. */
  saleOpensAt: string | null;
  presaleHours: number;
  depositCents: number;
};
