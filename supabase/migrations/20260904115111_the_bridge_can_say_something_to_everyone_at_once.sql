-- There was no way for an operator to tell members anything that was not a
-- pre-registered automated trigger. A venue change, weather across a whole
-- weekend, a city launch, a season going on sale — each was an engineer's job
-- or a migration. For a club running fifty-two episodes in two cities that
-- is a weekly need, and the primitives — notify_member, queue_email, the
-- notification fan-out to push — already existed and were used one member at
-- a time.
--
-- One write, one audience, one record of what was said and to how many.
...
create policy "the bridge reads what was said" on public.broadcasts
  for select to authenticated using (public.is_staff());
grant select on public.broadcasts to authenticated;

-- The letter the broadcast goes out as. Its body carries the title and the
-- text verbatim, which is why it is one template and not one per occasion.
insert into public.email_templates (code, description, active)
values ('bridge-word', 'A word from the Bridge to a chosen audience — an announcement, a change, a date. The body is written by the operator.', true)
...
revoke all on function public.send_broadcast(jsonb, text, text, text[]) from public, anon;
grant execute on function public.send_broadcast(jsonb, text, text, text[]) to authenticated;;
