-- The strike guard measured "money on the book" as cash owed, so a pass paid
-- entirely with plan credit read as nothing owed: the episode was struck
-- inside the window and the month's credit stayed spent. A spent credit is
-- the member's money too. Inside the window either kind refuses; outside it
-- both come back.
...
begin
  ...
  select coalesce(sum(l.delta_cents), 0) into plan_applied
    from public.account_ledger l where l.episode_id = old.id and l.kind = 'plan_credit';
  if owed <= 0 and plan_applied <= 0 then
    return old;
  end if;
