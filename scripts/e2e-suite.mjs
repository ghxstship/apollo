...
/* Every fixture this RUN creates carries this token, and the sweep only removes
   ...
   branch's — and both swept `episodes?slug=like.e2e-*`, which is every fixture
...
async function sweep(p) {
  ...
  /* Program fixtures. editions holds a RESTRICT on its template voyage,
     ...
     template with it. */
  for (const rel of ["editions", "seasons", "venues"]) {
  ...
  for (const rel of ["contests", "episodes"]) {
    await stf.del(`${rel}?slug=like.e2e-*${RUN_TOKEN}*`);
    await stf.del(`${rel}?slug=like.e2e-*&created_at=lt.${STALE_BEFORE()}`);
  }
  ...
  /* The waitlist notice CANNOT be swept, and saying so is better than a
     ...
     the same discipline knots_ledger keeps. So each run leaves exactly one
  ...
  await stf.del("tables?number=eq.99");
  ...
  await stf.del("series?slug=like.e2e-*");
...
/* Every relation anon can reach has to be named in one of these two lists, or
   it is covered by neither: `cabins` and `episode_cuts` were readable and appeared
   in neither, so nothing ever checked what they hand out. */
const ANON_READABLE = [
  "episodes", "cities", "vessels", "episode_vessels", "log_posts",
  "addons", "membership_plans", "crew_roles", "episode_capacity",
  "cabins", "episode_cuts",
  ...
  "series", "club_products", "episode_legs", "episode_stops",
];
const ANON_SEALED = [
  "profiles", "passes", "pass_guests", "pass_addons", "pass_transfers",
  "knots_ledger", "account_ledger", "notifications", "invites", "member_roll",
  "threads", "thread_members", "messages", "open_deck_posts", "open_deck_comments",
  "open_deck_hails", "open_deck_flags", "applications", "crew_candidates",
  ...
  "member_engagement", "member_affinity", "knots_balance", "account_balance",
...
async function anonSurface() {
  ...
  /* episode_media is NOT sealed and must not be listed as such — the public
  ...
  const frames = await anon.get("episode_media?select=id,approved,storage_path");
  ...
  const pending = await anon.get("episode_media?select=id&approved=eq.false");
  ...
  if (somePath) {
    const direct = await fetch(`${SUPA}/storage/v1/object/public/episode-media/${somePath}`);
  ...
  for (const [t, body] of [
    ["episodes", { slug: `e2e-anon-${stamp}`, title: "x", setting: "sea", starts_at: "2027-01-01" }],
    ["profiles", { full_name: "x" }],
    ["passes", { episode_id: "00000000-0000-0000-0000-000000000000", profile_id: "00000000-0000-0000-0000-000000000000" }],
    ["open_deck_posts", { body: "x" }],
    ...
    ["knots_ledger", { profile_id: "00000000-0000-0000-0000-000000000000", delta: 10000, reason: "x" }],
...
const OWNED_TABLES = [
  "knots_ledger", "account_ledger", "notifications", "passes", "subscriptions",
...
const OWNED_VIEWS = [
  "knots_balance", "account_balance", "member_engagement", "member_league",
  "member_pass_usage",
];
...
async function isolationRules(p) {
  ...
  const mint = await reg.post("knots_ledger", { profile_id: uid(p.regional), delta: 99999, reason: "E2E mint" });
...
async function moderationRules(p) {
  ...
  const post = await glo.post("open_deck_posts", { author_id: uid(p.global), body: "E2E — moderation fixture." });
  const pid = post.data?.[0]?.id;
  note("global", "posts to the Open Deck", Boolean(pid), `got ${post.status}`);
  if (!pid) return;

  const hail = await reg.post("open_deck_hails", { post_id: pid, profile_id: uid(p.regional) });
  note("regional", "may hail a post", hail.status < 400, `got ${hail.status}`);

  const forgeHail = await reg.post("open_deck_hails", { post_id: pid, profile_id: uid(p.global) });
  note("regional", "cannot hail as someone else", forgeHail.status >= 400, `got ${forgeHail.status}`);

  const comment = await reg.post("open_deck_comments", { post_id: pid, author_id: uid(p.regional), body: "E2E comment." });
  const cid = comment.data?.[0]?.id;
  note("regional", "may comment", comment.status < 400, `got ${comment.status}`);

  const forgeComment = await reg.post("open_deck_comments", { post_id: pid, author_id: uid(p.global), body: "E2E forged." });
  note("regional", "cannot comment under another name", forgeComment.status >= 400, `got ${forgeComment.status}`);

  if (cid) {
    const steal = await glo.patch(`open_deck_comments?id=eq.${cid}`, { body: "E2E rewritten." });
    const now = await reg.get(`open_deck_comments?id=eq.${cid}&select=body`);
    note("global", "cannot rewrite another member's comment", now.data?.[0]?.body !== "E2E rewritten.", `got ${steal.status}`);
  }

  const flag = await reg.post("open_deck_flags", { post_id: pid, flagger_id: uid(p.regional), reason: "E2E" });
  ...
  const readFlags = await reg.get("open_deck_flags?select=flagger_id&limit=20");
  ...
  await stf.del(`open_deck_flags?post_id=eq.${pid}`);
  await stf.del(`open_deck_comments?post_id=eq.${pid}`);
  await stf.del(`open_deck_hails?post_id=eq.${pid}`);
  await stf.del(`open_deck_posts?id=eq.${pid}`);
}
...
async function documentRules(p) {
  ...
  const guests = await stf.get("pass_guests?select=id,sign_token,name&rsvp_id=not.is.null&limit=1");
  ...
  const gv = await stf.get(
    `episodes?select=id&class=eq.sea&status=eq.scheduled&starts_at=gt.${new Date().toISOString()}&order=starts_at.asc&limit=1`
  );
  const gr = await stf.post("passes", {
    episode_id: gv.data?.[0]?.id, profile_id: uid(p.staff), status: "aboard",
  });
  const grId = gr.data?.[0]?.id;
  const gg = grId
    ? await stf.post("pass_guests", { rsvp_id: grId, name: "E2E Redaction Guest" })
    : { data: null };
  ...
  if (gId) await stf.del(`pass_guests?id=eq.${gId}`);
  if (grId) await stf.del(`passes?id=eq.${grId}`);
...
async function enforcementRules(p) {
  ...
  /* --- the gangway gate --- */
  const vres = await stf.get("episodes?select=id,class&status=eq.scheduled&class=eq.shore&limit=1");
  const vid = vres.data?.[0]?.id;
  const heldSeat = vid
    ? await stf.get(`passes?select=id&episode_id=eq.${vid}&profile_id=eq.${uid(p.staff)}&limit=1`)
    : { data: [] };
  const seat = heldSeat.data?.[0]
    ? { data: heldSeat.data }
    : vid
      ? await stf.post("passes", { episode_id: vid, profile_id: uid(p.staff), status: "aboard" })
      : { data: null };
  ...
  if (seatId) {
    /* The refusal is exercised on a guest rather than a member: a signature is
       ...
       triggers. */
    const fresh = await stf.post("pass_guests", { rsvp_id: seatId, name: `E2E Unsigned ${stamp}` });
    const freshId = fresh.data?.[0]?.id;
    if (freshId) {
      const refused = await stf.patch(`pass_guests?id=eq.${freshId}`, { checked_in_at: new Date().toISOString() });
      ...
      const removed = await stf.del(`pass_guests?id=eq.${freshId}`);
    ...
    const signedIn = await stf.patch(`passes?id=eq.${seatId}`, { checked_in_at: new Date().toISOString() });
    ...
    const existing = await stf.get(`pass_guests?select=id,sign_token,rsvp_id&name=eq.E2E Gate Guest&limit=1`);
    const g = existing.data?.[0]
      ? { data: existing.data }
      : await stf.post("pass_guests", { rsvp_id: seatId, name: "E2E Gate Guest" });
    const gId = g.data?.[0]?.id;
    const gTok = g.data?.[0]?.sign_token;
    /* The pass the guest rode on may be gone — a fixture reset strikes the
       personas' passes, and pass_guests.rsvp_id is SET NULL rather than
    ...
    if (gId && g.data[0].rsvp_id !== seatId) {
      await stf.patch(`pass_guests?id=eq.${gId}`, { rsvp_id: seatId });
    }
    if (gId) {
      await anon.rpc("sign_document_as_guest", {
        p_token: gTok, p_document_code: "guest-waiver", p_consent: true,
      ...
      const gSigned = await stf.patch(`pass_guests?id=eq.${gId}`, { checked_in_at: new Date().toISOString() });
      note("staff", "a signed guest boards", gSigned.status < 400, `got ${gSigned.status}`);

      const removeSigned = await stf.del(`pass_guests?id=eq.${gId}`);
      ...
      await stf.patch(`pass_guests?id=eq.${gId}`, { checked_in_at: null });
    }
    /* The seat carries the fixture guest, so it stays too — reset instead. */
    await stf.patch(`passes?id=eq.${seatId}`, { checked_in_at: null });
  }
  ...
  if (seatId) {
    await stf.patch(`passes?id=eq.${seatId}`, { status: "not_going" });
    await stf.patch(`passes?id=eq.${seatId}`, { status: "aboard" });
...
async function clubRules(p) {
  ...
  const stamp = `${Date.now().toString(36)}${RUN_TOKEN}`;
  const night = await stf.post("episodes", {
    slug: `e2e-table-night-${stamp}`, title: "E2E table night.", setting: "shore", kind: "port_day",
    starts_at: new Date(Date.now() + 3 * 864e5).toISOString(),
    passes_total: 24, price_cents: 0, min_tier: "regional",
  });
  const nightId = night.data?.[0]?.id;
  const tbl = await stf.post("tables", { episode_id: nightId, number: 99, seats: 2 });
  ...
  if (tblId) {
    ...
    for (const who of ["regional", "national"]) {
      await stf.post("passes", { episode_id: nightId, profile_id: uid(p[who]), status: "aboard" });
    }
    ...
    await stf.patch(`episodes?id=eq.${nightId}`, { starts_at: new Date(Date.now() - 3600e3).toISOString() });
    ...
    await stf.del(`tables?id=eq.${tblId}`);
  }
  if (nightId) await stf.del(`passes?episode_id=eq.${nightId}`);
  if (nightId) await stf.del(`episodes?id=eq.${nightId}`);
}
...
async function hardeningRules(p) {
  ...
  const foreignRsvp = await stf.get(`passes?profile_id=neq.${me}&select=id&limit=1`);
  ...
  const past = await stf.get("episodes?status=eq.completed&select=id&limit=1");
  if (past.data?.[0]?.id) {
    const back = await reg.post("passes", {
      episode_id: past.data[0].id, profile_id: me, status: "aboard",
    });
  ...
  const anyGuest = await stf.get(
    "pass_guests?select=sign_token&sign_token=not.is.null&rsvp_id=not.is.null&limit=1"
  );
  ...
  const myRsvps = await glo.get(`passes?profile_id=eq.${uid(p.global)}&select=id`);
  const mine = new Set((myRsvps.data || []).map((r) => r.id));
  const harvest = await glo.get("pass_guests?select=rsvp_id,sign_token&limit=20");
...
async function homeWater(stf) {
  const h = await stf.get("cities?slug=eq.miami&select=id&limit=1");
  const id = h.data?.[0]?.id;
  return id ? `&or=(city_id.is.null,city_id.eq.${id})` : "";
}
...
async function roundTwoRules(p) {
  ...
  if (v) {
    ...
    const folio = async (since) => {
      const rows = await reg.get(
        `account_ledger?profile_id=eq.${me}&episode_id=eq.${v.id}&created_at=gt.${since}&select=delta_cents`
      );
    ...
    const claim = () => reg.post("passes", { episode_id: v.id, profile_id: me, status: "aboard" });
    ...
    if (rid) await reg.del(`passes?id=eq.${rid}`);
    ...
    if (rid2) {
      ...
      await reg.patch(`passes?id=eq.${rid2}`, { status: "waitlist" });
      const mine = await reg.get(`passes?id=eq.${rid2}&select=status`);
      ...
      await stf.del(`passes?id=eq.${rid2}`);
    }
    await stf.del(`account_ledger?profile_id=eq.${me}&episode_id=eq.${v.id}`);
    await stf.del(`knots_ledger?profile_id=eq.${me}&episode_id=eq.${v.id}`);
  ...
  const gone = await stf.get("episodes?status=eq.completed&select=id&limit=1");
  if (gone.data?.[0]?.id) {
    const late = await reg.post("passes", {
      episode_id: gone.data[0].id, profile_id: me, status: "waitlist",
    });
  ...
  const clocks = await stf.get("episodes?select=slug,time_zone&limit=200");
  ...
  const goneGuest = await stf.get(
    "pass_guests?select=sign_token,passes!inner(episodes!inner(status))&passes.episodes.status=eq.completed&limit=1"
  );
  ...
  const camera = await stf.get("pass_guests?select=on_camera&limit=50");
...
async function roundFiveRules(p) {
  ...
  const ahead = await stf.get(
    `episodes?status=eq.scheduled&select=id&starts_at=gt.${new Date().toISOString()}${await homeWater(stf)}&order=starts_at.asc&limit=1`
  );
  ...
  const put = await fetch(`${SUPA}/storage/v1/object/episode-media/${path}`, {
    method: "POST",
  ...
  const signAs = async (session) => {
    const res = await fetch(`${SUPA}/storage/v1/object/sign/episode-media`, {
      method: "POST",
      headers: {
        apikey: ANON,
  ...
  const mk = await stf.post("episode_media", {
    episode_id: vid, storage_path: path, caption: "E2E consent frame",
  ...
  note("staff", "the Bridge signs its thumbnails",
    /object\/sign\/episode-media/.test(html), "no signed URL on the media screen");
  note("staff", "the Bridge builds no public URL against a private bucket",
    !/object\/public\/episode-media/.test(html), "found a /object/public/ URL");

  const selfClear = await glo.patch(`episode_media?id=eq.${fid}`, { approved: true });
  note("global", "cannot clear their own frame for the water", selfClear.status >= 400,
    `got ${selfClear.status}`);

  const bridgeClear = await stf.patch(`episode_media?id=eq.${fid}`, { approved: true });
  ...
  note("anon", "the gallery actually renders a cleared frame",
    /object\/sign\/episode-media/.test(gallery), "no signed frame on the gallery");
  ...
  const pull = await glo.patch(`episode_media?id=eq.${fid}`, { approved: false });
  ...
  await stf.patch(`episode_media?id=eq.${fid}`, { approved: true });
  const amend = await glo.patch(`episode_media?id=eq.${fid}`, { caption: "E2E second thoughts" });
  ...
  const move = await glo.patch(`episode_media?id=eq.${fid}`, { uploaded_by: uid(p.regional) });
  ...
  const strangerEdit = await reg.patch(`episode_media?id=eq.${fid}`, { caption: "not mine" });
  note("regional", "cannot edit a frame that is not theirs",
    (strangerEdit.data || []).length === 0, `got ${strangerEdit.status}`);
  const strangerDel = await reg.del(`episode_media?id=eq.${fid}`);
  ...
  const resting = await stf.get(`episode_media?id=eq.${fid}&select=storage_path`);
  ...
  const own = await glo.del(`episode_media?id=eq.${fid}`);
  ...
  await fetch(`${SUPA}/storage/v1/object/episode-media`, {
    method: "DELETE",
...
async function roundThreeRules(p) {
  ...
  if (v) {
    ...
    for (const [label, away] of [["not_going", "not_going"], ["waitlist", "waitlist"]]) {
      ...
      const rid = first.data?.[0]?.id;
      if (rid) {
        await reg.patch(`passes?id=eq.${rid}`, { status: away });
        const released = await folio(mark);
        await reg.patch(`passes?id=eq.${rid}`, { status: "aboard" });
    ...
    if (hid) {
      ...
      await reg.patch(`passes?id=eq.${hid}`, { show_on_manifest: false });
  ...
  if (v) {
    ...
    await stf.del(`passes?profile_id=eq.${gid}&episode_id=eq.${v.id}`);
    const seatG = await stf.post("passes", {
      episode_id: v.id, profile_id: gid, status: "aboard", comp: true,
      guests: 2, guest_names: ["E2E Slot One", "E2E Slot Two"],
    });
    const grid = seatG.data?.[0]?.id;
    if (grid) {
      await stf.patch(`passes?id=eq.${grid}`, {
        guests: 2, guest_names: ["E2E Slot Two", "E2E Slot Three"],
      });
      const party = await stf.get(`pass_guests?rsvp_id=eq.${grid}&select=name,boarding_code,sign_token`);
      ...
      await stf.del(`pass_guests?rsvp_id=eq.${grid}`);
      await stf.del(`passes?id=eq.${grid}`);
    }
    await stf.del(`account_ledger?profile_id=eq.${gid}&episode_id=eq.${v.id}`);
    await stf.del(`knots_ledger?profile_id=eq.${gid}&episode_id=eq.${v.id}`);
  }
      ...
      if (isStaffRoute && name !== "staff") {
        ...
        if (ok) {
          ...
          /* Against VISIBLE text, not raw HTML: a word like "berth" lives in
             RSC payloads as passes_total forever, and what matters is what a
             person reads. */
...
async function rebrandRules(p) {
  ...
  const idx = await stf.get("passes?select=boarding_code&boarding_code=not.is.null&limit=200");
...
async function logbookRules(p) {
  ...
  const paid = await reg.get(
    `knots_ledger?profile_id=eq.${uid(p.regional)}&reason=eq.${encodeURIComponent("Won — E2E draft contest.")}&order=created_at.desc&limit=1`
  );
...
async function parityRules(p) {
  ...
  const mkS = await stf.post("episodes", {
    slug: sslug, title: "E2E signal sailing.", setting: "sea", kind: "sea_day",
    starts_at: new Date(Date.now() + 45 * 864e5).toISOString(),
    passes_total: 4, price_cents: 0, min_tier: "regional", distance_nm: 5,
  });
  ...
  const board = await glo.post("passes", { episode_id: svid, profile_id: uid(p.global), status: "aboard" });
  note("global", "boards the fixture", board.status === 201, `got ${board.status}`);
  await new Promise((r) => setTimeout(r, 600));

  const crew = await glo.get(`threads?episode_id=eq.${svid}&kind=eq.crew&select=id`);
  ...
  const anyVoyage = await glo.get("episodes?select=id&limit=1");
  const vid = anyVoyage.data?.[0]?.id;
  const bad = await glo.rpc("check_promo", { p_code: "NOPE-NOT-A-CODE", p_episode: vid });
  note("global", "unknown code is refused", bad.data?.ok === false, JSON.stringify(bad.data));
  const good = await glo.rpc("check_promo", { p_code: "FOUNDING", p_episode: vid });
  note("global", "a live code validates", good.data?.ok === true && good.data?.kind === "percent", JSON.stringify(good.data));
  const anonCode = await rest(null).rpc("check_promo", { p_code: "FOUNDING", p_episode: vid });
  ...
  const guests = await stf.get(
    "pass_guests?select=name,boarding_code&boarding_code=not.is.null&name=not.like.E2E*&limit=3"
  );
  ...
  const nosyRsvps = await glo.get(`passes?profile_id=eq.${uid(p.global)}&select=id`);
  const own = new Set((nosyRsvps.data || []).map((r) => r.id));
  const nosy = await glo.get("pass_guests?select=id,rsvp_id&limit=20");
  ...
  const rmS = await stf.del(`episodes?id=eq.${svid}`);
...
async function businessRules(p) {
  ...
  const mk = await stf.post("episodes", {
    slug, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 30 * 864e5).toISOString(),
    passes_total: 1, price_cents: 1000, min_tier: "national", distance_nm: 10,
  });
  note("staff", "creates fixture voyage", mk.status === 201, `got ${mk.status} ${JSON.stringify(mk.data).slice(0, 120)}`);
  const vid = mk.data?.[0]?.id;
  if (!vid) return;

  // Member cannot create episodes
  const memberMk = await glo.post("episodes", { slug: slug + "-x", title: "Nope.", setting: "sea", starts_at: new Date().toISOString() });
  note("global", "cannot create episodes", memberMk.status === 403 || memberMk.status === 401, `got ${memberMk.status}`);

  // Tier gate: regional blocked from national sailing
  const regTry = await reg.post("passes", { episode_id: vid, profile_id: uid(p.regional), status: "aboard" });
  ...
  const pauTry = await pau.post("passes", { episode_id: vid, profile_id: uid(p.paused), status: "aboard" });
  ...
  const natLedger = await nat.get(`account_ledger?episode_id=eq.${vid}&kind=eq.pass&select=delta_cents`);
  note("national", "pass charge posts to house account", natLedger.data?.[0]?.delta_cents === -1000, JSON.stringify(natLedger.data));
  const natCode = await nat.get(`passes?episode_id=eq.${vid}&profile_id=eq.${uid(p.national)}&select=boarding_code`);
  ...
  const gloWait = await glo.post("passes", { episode_id: vid, profile_id: uid(p.global), status: "waitlist" });
  ...
  const natRel = await nat.del(`passes?episode_id=eq.${vid}&profile_id=eq.${uid(p.national)}`);
  note("national", "releases the berth", natRel.status === 200 || natRel.status === 204, `got ${natRel.status}`);
  await new Promise((r) => setTimeout(r, 400));
  const gloNow = await glo.get(`passes?episode_id=eq.${vid}&profile_id=eq.${uid(p.global)}&select=status`);
  note("global", "auto-promoted from waitlist in order", gloNow.data?.[0]?.status === "aboard", JSON.stringify(gloNow.data));
  const natCredit = await nat.get(`account_ledger?episode_id=eq.${vid}&kind=eq.credit&select=delta_cents`);
  ...
  await stf.patch(`episodes?id=eq.${vid}`, { status: "completed" });
  await new Promise((r) => setTimeout(r, 400));
  const gloFm = await glo.get(`knots_ledger?episode_id=eq.${vid}&select=delta,reason`);
  ...
  /* The balance redeem_reward actually tests is the sum of knots_ledger,
  ...
  const bal = await reg.get(`knots_balance?profile_id=eq.${uid(p.regional)}&select=balance`);
  ...
  const post = await glo.post("open_deck_posts", { author_id: uid(p.global), body: "E2E — the fixture speaks." });
  const pid = post.data?.[0]?.id;
  note("global", "posts to the wardroom", post.status === 201, `got ${post.status}`);
  const regDel = await reg.del(`open_deck_posts?id=eq.${pid}`);
  const still = await glo.get(`open_deck_posts?id=eq.${pid}&select=id`);
  note("regional", "cannot delete another member's post", (still.data || []).length === 1, `del ${regDel.status}`);
  const flag = await reg.post("open_deck_flags", { post_id: pid, flagger_id: uid(p.regional), reason: "conduct — e2e" });
  note("regional", "can flag for the harbormaster", flag.status === 201, `got ${flag.status}`);
  const stfDel = await stf.del(`open_deck_posts?id=eq.${pid}`);
  ...
  /* Cleanup fixture. The ledger rows CANNOT go with it: knots_ledger has a
  ...
  const rm = await stf.del(`episodes?id=eq.${vid}`);
...
/* ---------- F. Vetting, Radar and Show ----------
   ...
   trigger is inert on every sailing with no episode_radar row, and proving it
...
async function ratioAndRadarRules(p) {
  ...
  const mk = async (slug, berths) => {
    const v = await stf.post("episodes", {
      slug, title: `E2E ${slug}`, setting: "sea", kind: "sea_day", sub_class: "passage",
      starts_at: soon, passes_total: berths, status: "live", min_tier: "regional",
      time_zone: "America/New_York",
    });
  ...
  const ungated = await reg.post("passes", { episode_id: ratioVid, profile_id: uid(p.regional), status: "aboard" });
  note("regional", "an ungated sailing takes a pass with no segment", ungated.status === 201, `got ${ungated.status}`);
  await stf.del(`passes?episode_id=eq.${ratioVid}&profile_id=eq.${uid(p.regional)}`);

  const caps = await stf.post("episode_segment_caps", [
    { episode_id: ratioVid, segment: "single_woman", cap: 1 },
    { episode_id: ratioVid, segment: "single_man", cap: 1 },
    { episode_id: ratioVid, segment: "couple", cap: 3 },
  ]);
  ...
  const unvetted = await reg.post("passes", { episode_id: ratioVid, profile_id: uid(p.regional), status: "aboard", segment: "single_man" });
  ...
  const noSeg = await reg.post("passes", { episode_id: ratioVid, profile_id: uid(p.regional), status: "aboard" });
  ...
  const seatMan = await reg.post("passes", { episode_id: ratioVid, profile_id: uid(p.regional), status: "aboard", segment: "single_man" });
  ...
  const secondMan = await nat.post("passes", { episode_id: ratioVid, profile_id: uid(p.national), status: "aboard", segment: "single_man" });
  ...
  const line = await nat.post("waitlist_entries", { episode_id: ratioVid, profile_id: uid(p.national), segment: "single_man", place: 99 });
  ...
  const couple = await glo.post("passes", { episode_id: ratioVid, profile_id: uid(p.global), status: "aboard", segment: "couple" });
  note("global", "takes a couple pass", couple.status === 201, `got ${couple.status} ${said(couple).slice(0, 80)}`);

  const cap = await anon.get(`episode_segment_capacity?episode_id=eq.${ratioVid}&select=segment,cap,units,remaining`);
  ...
  const overHull = await stf.post("passes", { episode_id: ratioVid, profile_id: uid(p.staff), status: "aboard", segment: "couple" });
  ...
  const withGuest = await stf.patch(`passes?episode_id=eq.${ratioVid}&profile_id=eq.${uid(p.global)}`, { guests: 1 });
  ...
  const offerFull = await stf.rpc("offer_the_next_place", { p_episode: ratioVid, p_segment: "single_man" });
  ...
  const offered = await stf.rpc("offer_the_next_place", { p_episode: ratioVid, p_segment: "single_man" });
  ...
  const nowAboard = await stf.get(`passes?episode_id=eq.${ratioVid}&profile_id=eq.${uid(p.national)}&select=status,segment`);
  ...
  const rcaps = await stf.post("episode_segment_caps", [
    { episode_id: radarVid, segment: "single_woman", cap: 10 },
    { episode_id: radarVid, segment: "single_man", cap: 10 },
    { episode_id: radarVid, segment: "couple", cap: 10 },
  ]);
  ...
  for (const [who, seg] of [["regional", "single_man"], ["global", "couple"], ["paused", "single_woman"]]) {
    const r = await stf.post("passes", { episode_id: radarVid, profile_id: uid(p[who]), status: "aboard", segment: seg });
    ...
    if (aboard[who]) await stf.patch(`passes?id=eq.${aboard[who]}`, { checked_in_at: new Date().toISOString() });
  }
  ...
  const clock = await stf.post("episode_radar", {
    episode_id: radarVid,
  ...
  const sweepRes = await reg.rpc("radar_sweep", { p_episode: radarVid });
  ...
  const ashore = await nat.rpc("radar_sweep", { p_episode: radarVid });
  note("national", "the sweep is refused to anyone not aboard", ashore.status >= 400 && /aboard/.test(said(ashore)), said(ashore).slice(0, 90));

  const pick1 = await reg.post("radar_picks", { episode_id: radarVid, picker_rsvp: aboard.regional, picked_rsvp: aboard.global });
  note("regional", "plots a course", pick1.status === 201, `got ${pick1.status} ${said(pick1).slice(0, 80)}`);
  const selfPick = await reg.post("radar_picks", { episode_id: radarVid, picker_rsvp: aboard.regional, picked_rsvp: aboard.regional });
  ...
  const overSlots = await reg.post("radar_picks", { episode_id: radarVid, picker_rsvp: aboard.regional, picked_rsvp: aboard.paused });
  ...
  const beforeMutual = await reg.get(`shared_anchors?episode_id=eq.${radarVid}&select=id`);
  note("regional", "a one-sided pick surfaces nothing", (beforeMutual.data || []).length === 0, JSON.stringify(beforeMutual.data).slice(0, 80));
  const spy = await glo.get(`radar_picks?episode_id=eq.${radarVid}&select=picked_rsvp`);
  note("global", "cannot read another member's picks", (spy.data || []).length === 0, JSON.stringify(spy.data).slice(0, 80));
  const staffSpy = await stf.get(`radar_picks?episode_id=eq.${radarVid}&select=picked_rsvp`);
  ...
  const back = await glo.post("radar_picks", { episode_id: radarVid, picker_rsvp: aboard.global, picked_rsvp: aboard.regional });
  note("global", "plots back", back.status === 201, `got ${back.status} ${said(back).slice(0, 80)}`);
  const sealed = await reg.get(`shared_anchors?episode_id=eq.${radarVid}&select=id`);
  ...
  const anchorExists = await stf.get(`shared_anchors?episode_id=eq.${radarVid}&select=id,unlocked_at`);
  ...
  /* The envelope. Before 19:00 it refuses by name; the wrong envelope refuses
     as someone else's. */
  const issued = await stf.rpc("issue_the_envelopes", { p_episode: radarVid });
  ...
  await stf.patch(`episode_radar?episode_id=eq.${radarVid}`, { anchors_unlock_at: new Date(nowMs - 60_000).toISOString() });
  ...
  const nowVisible = await reg.get(`shared_anchors?episode_id=eq.${radarVid}&select=id`);
  ...
  const extend = await stf.patch(`shared_anchors?episode_id=eq.${radarVid}`, { expires_at: new Date(nowMs + 200_000_000).toISOString() });
  ...
  const cutShort = await stf.patch(`shared_anchors?episode_id=eq.${radarVid}`, { expires_at: new Date(nowMs - 60_000).toISOString() });
  note("staff", "an anchor can be cut short", cutShort.status < 400, `got ${cutShort.status} ${said(cutShort).slice(0, 70)}`);
  const expired = await reg.get(`shared_anchors?episode_id=eq.${radarVid}&select=id`);
  ...
  await stf.patch(`episode_radar?episode_id=eq.${radarVid}`, { locks_at: new Date(nowMs - 60_000).toISOString() });
  const lateInsert = await glo.post("radar_picks", { episode_id: radarVid, picker_rsvp: aboard.global, picked_rsvp: aboard.paused });
  ...
  const lateDelete = await reg.del(`radar_picks?episode_id=eq.${radarVid}&picker_rsvp=eq.${aboard.regional}&picked_rsvp=eq.${aboard.global}`);
  const survived = await reg.get(`radar_picks?episode_id=eq.${radarVid}&picker_rsvp=eq.${aboard.regional}&picked_rsvp=eq.${aboard.global}&select=picked_rsvp`);
  ...
  const pod = await stf.post("pod_sessions", { episode_id: radarVid, rsvp_id: aboard.regional, position: 1, state: "waiting", blur_required: false });
  ...
  const podPrivate = await glo.get(`pod_sessions?episode_id=eq.${radarVid}&select=id`);
  ...
  /* ---- cleanup ----
     ...
     may have dropped episode_radar first, so the trigger has to read a missing
     ...
     but when the row disappears as a CASCADE from `episodes` the reversal does
  ...
  await stf.del(`passes?episode_id=eq.${ratioVid}`);
  await stf.del(`passes?episode_id=eq.${radarVid}`);
  const rmA = await stf.del(`episodes?id=eq.${ratioVid}`);
  const rmB = await stf.del(`episodes?id=eq.${radarVid}`);
  await stf.del(`elements?element_id=like.E2E-*`);
  for (const who of ["regional", "national", "global", "paused", "staff"]) {
  ...
  const leftV = await stf.get(`episodes?id=eq.${ratioVid}&select=id`);
  const leftR = await stf.get(`episodes?id=eq.${radarVid}&select=id`);
  ...
  note("staff", "the fixtures are gone and verified gone",
    ...
    (leftP.data || []).length === 0,
    `episodes ${(leftV.data || []).length + (leftR.data || []).length}, files ${(leftF.data || []).length}, elements ${(leftE.data || []).length}, sheets ${(leftP.data || []).length}, del ${rmA.status}/${rmB.status}`);
}

/* Read through staff so a paused persona's own read policy cannot skew it.

   Through knots_balance, NOT by summing knots_ledger. PostgREST caps a
   ...
   knots_balance is one row per member, aggregated in the database, and it is
...
async function remediationRules(p) {
  ...
  const smk = await stf.post("episodes", {
    slug: `e2e-course-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 30 * 24 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 6, status: "scheduled", price_cents: 0,
  });
  const svid = smk.data?.[0]?.id;
  note("staff", "remediation fixture sails", Boolean(svid), JSON.stringify(smk.data).slice(0, 120));
  const toDone = await stf.patch(`episodes?id=eq.${svid}`, { status: "completed" });
  note("staff", "scheduled may complete", toDone.status < 300, `got ${toDone.status}`);
  const backAlive = await stf.patch(`episodes?id=eq.${svid}`, { status: "live" });
  ...
  // — holds fit the hull —
  const smk2 = await stf.post("episodes", {
    slug: `e2e-holds-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 30 * 24 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 6, held_passes: 2, status: "scheduled", price_cents: 0,
  });
  const hvid = smk2.data?.[0]?.id;
  const overHold = await stf.patch(`episodes?id=eq.${hvid}`, { held_passes: 7 });
  ...
  const underBerth = await stf.patch(`episodes?id=eq.${hvid}`, { passes_total: 1 });
  ...
  const kpass = await reg.post("passes", { episode_id: hvid, profile_id: uid(p.regional), status: "aboard" });
  note("regional", "boards the doomed sailing", kpass.status === 201, `got ${kpass.status}`);
  await stf.del(`episodes?id=eq.${hvid}`);
  ...
  const tmk = await stf.post("episodes", {
    slug: `e2e-hand-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 30 * 24 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 6, status: "scheduled", price_cents: 1000,
  });
  const tvid = tmk.data?.[0]?.id;
  const aPass = await nat.post("passes", { episode_id: tvid, profile_id: uid(p.national), status: "aboard" });
  ...
  const stays = await nat.get(`passes?id=eq.${aRsvp}&select=profile_id`);
  ...
  note("global", "an acceptance cannot be hand-written", forged.status >= 400 || (await glo.get(`passes?id=eq.${aRsvp}&select=profile_id`)).data?.[0]?.profile_id === uid(p.national), `got ${forged.status}`);
  ...
  const tdel = await stf.del(`episodes?id=eq.${tvid}`);
  ...
  const dmk = await stf.post("episodes", {
    slug: `e2e-deposit-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 30 * 24 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 6, status: "scheduled", price_cents: 0, deposit_required: true,
  });
  const dvid = dmk.data?.[0]?.id;
  const shown = await reg.post("passes", { episode_id: dvid, profile_id: uid(p.regional), status: "aboard" });
  await nat.post("passes", { episode_id: dvid, profile_id: uid(p.national), status: "aboard" });
  note("regional", "the deposit is taken at booking", (await reg.get(`account_ledger?episode_id=eq.${dvid}&kind=eq.deposit&select=delta_cents`)).data?.[0]?.delta_cents === -5000, "");
  await stf.patch(`passes?id=eq.${shown.data?.[0]?.id}`, { checked_in_at: new Date().toISOString() });
  await stf.patch(`episodes?id=eq.${dvid}`, { status: "completed" });
  const back = await reg.get(`account_ledger?episode_id=eq.${dvid}&kind=eq.credit&memo=like.Deposit returned aboard*&select=delta_cents`);
  note("regional", "the deposit comes back aboard", back.data?.[0]?.delta_cents === 5000, JSON.stringify(back.data));
  const kept = await nat.get(`account_ledger?episode_id=eq.${dvid}&kind=eq.credit&memo=like.Deposit returned aboard*&select=delta_cents`);
  ...
  const ddel = await stf.del(`episodes?id=eq.${dvid}`);
  ...
  const wmk = await stf.post("episodes", {
    slug: `e2e-noauto-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 30 * 24 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 1, status: "scheduled", price_cents: 0,
  });
  const wvid = wmk.data?.[0]?.id;
  const seat = await glo.post("passes", { episode_id: wvid, profile_id: uid(p.global), status: "aboard" });
  note("global", "holds the seat to be freed", seat.status === 201, `got ${seat.status} ${JSON.stringify(seat.data).slice(0,100)}`);
  const wl1 = await reg.post("passes", { episode_id: wvid, profile_id: uid(p.regional), status: "waitlist", auto_claim: false });
  const wl2 = await nat.post("passes", { episode_id: wvid, profile_id: uid(p.national), status: "waitlist" });
  note("regional", "queues with the toggle off", wl1.status === 201 && wl2.status === 201, `got ${wl1.status}/${wl2.status}`);
  await glo.del(`passes?id=eq.${seat.data?.[0]?.id}`);
  const regStill = await reg.get(`passes?episode_id=eq.${wvid}&profile_id=eq.${uid(p.regional)}&select=status`);
  ...
  const natNow = await nat.get(`passes?episode_id=eq.${wvid}&profile_id=eq.${uid(p.national)}&select=status`);
  note("national", "the line moves past a no", natNow.data?.[0]?.status === "aboard", JSON.stringify(natNow.data));
  const wdel = await stf.del(`episodes?id=eq.${wvid}`);
  ...
  const amk = await stf.post("episodes", {
    slug: `e2e-addons-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 30 * 24 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 6, status: "scheduled", price_cents: 0,
  });
  const avid = amk.data?.[0]?.id;
  const apass = await reg.post("passes", { episode_id: avid, profile_id: uid(p.regional), status: "aboard" });
  const shelf = await reg.get("addons?select=id&limit=1");
  if (shelf.data?.[0]?.id) {
    const att = await reg.rpc("attach_addons", { p_pass: apass.data?.[0]?.id, p_addons: [shelf.data[0].id], p_qty: 1 });
    note("regional", "attaches an add-on through the folio", att.status < 300, `got ${att.status} ${JSON.stringify(att.data).slice(0, 120)}`);
    const arow = await reg.get(`account_ledger?episode_id=eq.${avid}&kind=eq.addon&select=delta_cents`);
  ...
  const adel = await stf.del(`episodes?id=eq.${avid}`);
  ...
  const pushmk = await stf.post("episodes", {
    slug: `e2e-onepush-${stamp}`, title: `E2E one-push ${stamp}`, setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 30 * 24 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 4, status: "scheduled", price_cents: 0,
  });
  const pushvid = pushmk.data?.[0]?.id;
  await glo.post("passes", { episode_id: pushvid, profile_id: uid(p.global), status: "aboard" });
  ...
  const frame = await glo.post("episode_media", {
    episode_id: pushvid, storage_path: `${uid(p.global)}/${pushvid}/e2e-frame.jpg`,
  ...
  const fdel2 = await glo.del(`episode_media?id=eq.${frame.data?.[0]?.id}`);
  note("global", "and may withdraw it", fdel2.status < 300, `got ${fdel2.status} ${JSON.stringify(fdel2.data).slice(0, 80)}`);
  await stf.patch(`episodes?id=eq.${pushvid}`, { status: "cancelled" });
  ...
  const pushdel = await stf.del(`episodes?id=eq.${pushvid}`);
  ...
  const opmk = await stf.post("episodes", {
    slug: `e2e-forty-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 30 * 24 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 40, status: "scheduled", price_cents: 0,
  });
  const opvid = opmk.data?.[0]?.id;
  const overCap = await stf.post("episode_segment_caps", [
    { episode_id: opvid, segment: "single_man", cap: 20 },
    { episode_id: opvid, segment: "single_woman", cap: 20 },
    { episode_id: opvid, segment: "couple", cap: 1 },
  ]);
  note("staff", "forty-two heads are refused", overCap.status >= 400 && /hull holds/.test(JSON.stringify(overCap.data)), `got ${overCap.status}`);
  const atCap = await stf.post("episode_segment_caps", [
    { episode_id: opvid, segment: "single_man", cap: 20 },
    { episode_id: opvid, segment: "single_woman", cap: 20 },
  ]);
  note("staff", "the twenty-twenty composition stands", atCap.status === 201, `got ${atCap.status} ${JSON.stringify(atCap.data).slice(0, 120)}`);
  await stf.del(`episodes?id=eq.${opvid}`);
  ...
  if (sheetRow.data?.[0]?.completed_at) {
    const vmk = await stf.post("episodes", {
      slug: `e2e-sheetgate-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
      starts_at: new Date(Date.now() + 30 * 24 * 3600e3).toISOString(),
      time_zone: "America/New_York", passes_total: 6, status: "scheduled", price_cents: 0,
    });
    const vvid = vmk.data?.[0]?.id;
    await stf.post("episode_segment_caps", [{ episode_id: vvid, segment: "single_man", cap: 6 }]);
    const held = sheetRow.data[0].completed_at;
    await reg.patch(`preference_sheets?profile_id=eq.${uid(p.regional)}`, { completed_at: null });
    const refused = await reg.post("passes", { episode_id: vvid, profile_id: uid(p.regional), status: "aboard", segment: "single_man" });
    ...
    await stf.del(`episodes?id=eq.${vvid}`);
  } else {
  ...
  const hmk = await stf.post("episodes", {
    slug: `e2e-smshold-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 30 * 24 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 6, status: "scheduled", price_cents: 0,
  });
  const hvid2 = hmk.data?.[0]?.id;
  await reg.post("passes", { episode_id: hvid2, profile_id: uid(p.regional), status: "aboard" });
  await reg.patch(`profiles?id=eq.${uid(p.regional)}`, { notification_prefs: { weather: false, berths: true, fathoms: true, digest: true } });
  await stf.patch(`episodes?id=eq.${hvid2}`, { status: "weather_hold" });
  ...
  const hdel = await stf.del(`episodes?id=eq.${hvid2}`);
  ...
  const post = await glo.post("open_deck_posts", { author_id: uid(p.global), body: `E2E remediation post ${stamp}` });
  const postId = post.data?.[0]?.id;
  const flag = await reg.post("open_deck_flags", { post_id: postId, flagger_id: uid(p.regional), reason: "E2E remediation flag" });
  const flagId = flag.data?.[0]?.id;
  note("regional", "raises the flag", flag.status === 201, `got ${flag.status}`);
  await stf.del(`open_deck_posts?id=eq.${postId}`);
  const orphanFlag = await stf.get(`open_deck_flags?id=eq.${flagId}&select=post_id`);
  ...
  const resFlagRow = await stf.get(`open_deck_flags?reason=eq.${encodeURIComponent("conduct — e2e")}&select=id,status&limit=1`);
  const resFlagId = resFlagRow.data?.[0]?.id;
  const resSettle = resFlagId ? await stf.patch(`open_deck_flags?id=eq.${resFlagId}`, { status: "left_up" }) : { status: 999 };
  const resSettled = resFlagId ? await stf.get(`open_deck_flags?id=eq.${resFlagId}&select=status`) : { data: [] };
  ...
  if (resFlagId) await stf.del(`open_deck_flags?id=eq.${resFlagId}`);
  await stf.del(`open_deck_flags?id=eq.${flagId}`);
...
async function knotsFor(person, staffSession) {
  ...
  const res = await s.get(`knots_balance?profile_id=eq.${uid(person)}&select=balance`);
...
async function activityRules(p) {
  ...
  const priced = await stf.post("series", {
  ...
  const unpriced = await stf.post("series", {
  ...
  const vy = await stf.post("episodes", {
    slug: `e2e-activity-${stamp}`, title: "E2E activity gate", setting: "shore",
    kind: "port_day", starts_at: new Date(Date.now() + 2 * 864e5).toISOString(),
    passes_total: 12, price_cents: 0, min_tier: "global", status: "scheduled",
    ...
    knots_multiplier: 0, distance_nm: 0,
  });
  const vid = vy.data?.[0]?.id;
  note("staff", "activity fixture sailing exists", !!vid, `got ${vy.status}`);
  if (vid) {
    const bookVetted = await reg.post("passes", { episode_id: vid, profile_id: uid(p.regional), status: "aboard" });
    ...
    await stf.patch(`episodes?id=eq.${vid}`, { series: "beach_day" });
    const bookPort = await reg.post("passes", { episode_id: vid, profile_id: uid(p.regional), status: "aboard" });
    ...
    await stf.patch(`episodes?id=eq.${vid}`, { series: null });
    const backOn = await reg.post("passes", { episode_id: vid, profile_id: uid(p.regional), status: "aboard" });
    ...
    await stf.del(`passes?episode_id=eq.${vid}`);
    await stf.del(`episodes?id=eq.${vid}`);
  }
  ...
  const cat = await anon.get("series?select=slug&limit=1");
  note("anon", "the format catalogue is readable", cat.status === 200 && (cat.data || []).length > 0,
    `got ${cat.status}`);
  const rewrite = await nat.patch("series?slug=eq.beach_day", { price_cents: 1 });
...
async function charterRules(p) {
  ...
  const cab = await stf.post("cabins", {
    vessel_id: vesselId, name: `E2E Cabin ${stamp}`, sleeps: 1, position: 99,
    deck: "Upper", side: "port", muster: "Station B",
  });
  const cabinId = cab.data?.[0]?.id;
  const vy = await stf.post("episodes", {
    slug: `e2e-charter-${stamp}`, title: "E2E passage", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 3 * 864e5).toISOString(),
    passes_total: 12, price_cents: 0, min_tier: "regional", status: "scheduled",
    knots_multiplier: 0, distance_nm: 0,
  });
  const vid = vy.data?.[0]?.id;
  if (vesselId && vid) await stf.post("episode_vessels", { episode_id: vid, vessel_id: vesselId });
  ...
  /* --- the 72-hour option --- */
  const hold = await reg.rpc("hold_a_cabin_on_option", { p_episode: vid, p_cabin: cabinId });
  ...
  const twice = await reg.rpc("hold_a_cabin_on_option", { p_episode: vid, p_cabin: cabinId });
  ...
  const cut = await nat.post("passes", { episode_id: vid, profile_id: uid(p.national), status: "aboard", cabin_id: cabinId });
  ...
  const peek = await nat.get(`charter_options?episode_id=eq.${vid}&select=profile_id`);
  note("national", "another member's hold is not readable", (peek.data || []).length === 0,
    JSON.stringify(peek.data ?? "").slice(0, 90));
  const counted = await nat.rpc("cabin_places_open", { p_episode: vid });
  ...
  const opt = await reg.get(`charter_options?episode_id=eq.${vid}&select=id&released_at=is.null&confirmed_at=is.null`);
  ...
  /* Asserted as "the cabin no longer refuses", not as "the booking succeeded":
     ...
     must not be. */
  const after = await nat.post("passes", { episode_id: vid, profile_id: uid(p.national), status: "aboard", cabin_id: cabinId });
  ...
  const forge = await reg.post("charter_options", {
    episode_id: vid, profile_id: uid(p.regional), cabin_id: cabinId,
  ...
  const leg = await stf.post("episode_legs", { episode_id: vid, day: 1, place: "E2E Palma", note: "lines off 20:00" });
  const legId = leg.data?.[0]?.id;
  note("staff", "a leg is posted", !!legId, `got ${leg.status}`);

  const halfHold = await stf.patch(`episode_legs?id=eq.${legId}`, {
  ...
  /* The line this module exists to hold. A leg hold is not a sailing hold:
     episodes.status fires handle_voyage_status(), which mails every aboard and
     waitlisted member and, on the adjacent branch, moves money. */
  const sail = await stf.get(`episodes?id=eq.${vid}&select=status`);
  ...
  const legNow = await stf.get(`episode_legs?id=eq.${legId}&select=status,hold_reason`);
  ...
  /* A stop cannot be filed under a leg of another passage — a composite key,
     not a comment. */
  const otherVy = await stf.post("episodes", {
    slug: `e2e-charter-other-${stamp}`, title: "E2E other passage", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 4 * 864e5).toISOString(), passes_total: 4, price_cents: 0,
    knots_multiplier: 0, distance_nm: 0,
  });
  const otherId = otherVy.data?.[0]?.id;
  const strayStop = await stf.post("episode_stops", {
    episode_id: otherId, leg_id: legId, position: 1, name: "E2E stray stop",
  });
  note("staff", "a stop cannot belong to another passage's leg", strayStop.status >= 400,
    `got ${strayStop.status}`);
  if (otherId) await stf.del(`episodes?id=eq.${otherId}`);
...
async function programRules(p) {
  ...
  const dmk = await stf.post("episodes", {
    slug: `e2e-drop-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 6, status: "scheduled",
  ...
  const dropReg = await reg.post("passes", { episode_id: dropVid, profile_id: uid(p.regional), status: "aboard" });
  ...
  const dropNat = await nat.post("passes", { episode_id: dropVid, profile_id: uid(p.national), status: "aboard" });
  ...
  const dropGlo = await glo.post("passes", { episode_id: dropVid, profile_id: uid(p.global), status: "aboard" });
  note("global", "the deepest tier walks in first", dropGlo.status === 201, `got ${dropGlo.status} ${said(dropGlo).slice(0, 100)}`);
  const dropDep = await glo.get(`account_ledger?episode_id=eq.${dropVid}&kind=eq.deposit&select=delta_cents`);
  note("global", "the deposit is the sailing's own figure", dropDep.data?.[0]?.delta_cents === -12000, JSON.stringify(dropDep.data));
  const badFmt = await stf.post("episodes", {
    slug: `e2e-badfmt-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 4, series: "not_a_format",
  });
  note("staff", "a format answers to the catalogue", badFmt.status >= 400, `got ${badFmt.status}`);
  const dropDel = await stf.del(`episodes?id=eq.${dropVid}`);
  ...
  const tmk = await stf.post("episodes", {
    slug: `e2e-sert-${stamp}`, title: "E2E series sailing.", setting: "sea", kind: "voyage",
    starts_at: new Date(Date.now() + 40 * 24 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 4, status: "scheduled", price_cents: 0, series: "sandbar",
  });
  const tplVid = tmk.data?.[0]?.id;
  const smk = await stf.post("editions", {
    slug: `e2e-ser-${stamp}`, title: "E2E weekly", cadence_days: 7, template_episode_id: tplVid,
  });
  ...
  const serMember = await glo.post("editions", { slug: `e2e-serx-${stamp}`, title: "Nope", cadence_days: 7, template_episode_id: tplVid });
  ...
  const occ = await stf.get(`episodes?edition_id=eq.${serId}&select=id,slug,series,deposit_cents&order=starts_at`);
  note("staff", "the occurrences inherit the template's program",
    (occ.data ?? []).length === 2 && occ.data.every((o) => o.series === "sandbar"),
  ...
  const occDel = await stf.del(`episodes?edition_id=eq.${serId}`);
  const serDel = await stf.del(`editions?id=eq.${serId}`);
  const tplDel = await stf.del(`episodes?id=eq.${tplVid}`);
  ...
  const bmk = await stf.post("episodes", {
    slug: `e2e-daybed-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 6, status: "scheduled", price_cents: 0,
  });
  const bedVid = bmk.data?.[0]?.id;
  const bedReg = await reg.post("passes", { episode_id: bedVid, profile_id: uid(p.regional), status: "aboard" });
  const bedNat = await nat.post("passes", { episode_id: bedVid, profile_id: uid(p.national), status: "aboard" });
  const bedGlo = await glo.post("passes", { episode_id: bedVid, profile_id: uid(p.global), status: "aboard" });
  const claim1 = await reg.rpc("claim_a_daybed", { p_pass: bedReg.data?.[0]?.id });
  note("regional", "a daybed rides the pass", claim1.status < 300, `got ${claim1.status} ${said(claim1).slice(0, 90)}`);
  const bedCharge = await reg.get(`account_ledger?episode_id=eq.${bedVid}&kind=eq.addon&select=delta_cents,memo`);
  ...
  const claimTwice = await reg.rpc("claim_a_daybed", { p_pass: bedReg.data?.[0]?.id });
  note("regional", "one daybed to a pass", claimTwice.status >= 400, `got ${claimTwice.status}`);
  const claim2 = await nat.rpc("claim_a_daybed", { p_pass: bedNat.data?.[0]?.id });
  const claim3 = await glo.rpc("claim_a_daybed", { p_pass: bedGlo.data?.[0]?.id });
  ...
  const bedMine = await nat.get(`episode_daybeds?episode_id=eq.${bedVid}&select=id`);
  note("national", "the daybed list shows your own name only", (bedMine.data ?? []).length === 1, JSON.stringify(bedMine.data).slice(0, 80));
  const bedDel = await stf.del(`episodes?id=eq.${bedVid}`);
  ...
  const cabMk = await stf.post("cabins", { vessel_id: hullId, name: "E2E Owner", sleeps: 2, premium_cents: 4000 });
  const cabId = cabMk.data?.[0]?.id;
  const cmk2 = await stf.post("episodes", {
    slug: `e2e-cabin-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 4, status: "scheduled", price_cents: 0,
  });
  const cabVid = cmk2.data?.[0]?.id;
  await stf.post("episode_vessels", { episode_id: cabVid, vessel_id: hullId });
  const cabPass = await glo.post("passes", { episode_id: cabVid, profile_id: uid(p.global), status: "aboard" });
  const cabRsvp = cabPass.data?.[0]?.id;
  const takeCab = await glo.patch(`passes?id=eq.${cabRsvp}`, { cabin_id: cabId });
  ...
  await glo.patch(`passes?id=eq.${cabRsvp}`, { cabin_id: null });
  ...
  const cabDel = await stf.del(`episodes?id=eq.${cabVid}`);
  ...
  const prMk = await reg.post("member_event_proposals", {
    proposer_id: uid(p.regional), title: `E2E gathering ${stamp}`, series: "gathering", note: "E2E",
  });
  ...
  const spmk2 = await stf.post("episodes", {
    slug: `e2e-spon-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "voyage",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 4, status: "scheduled", price_cents: 0,
  ...
  await stf.post("episode_sponsors", { episode_id: sponVid, sponsor_id: spId });
  const credits = await anon.rpc("sponsor_credits", { p_episode: sponVid });
  ...
  const sponDel = await stf.del(`episodes?id=eq.${sponVid}`);
...
async function crawlRules(p) {
  ...
  const incl = await stf.post("episodes", {
    slug: `e2e-incl-${stamp}`, title: "E2E fixture sailing.", setting: "shore", kind: "port_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 4, series: "shore_leave", price_cents: 1000,
  });
  note("staff", "an included format is never sold", incl.status >= 400 && /included|never sold/.test(said(incl)), said(incl).slice(0, 100));
  const overCap = await stf.post("episodes", {
    slug: `e2e-over-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 41, series: "sandbar", price_cents: 0,
  });
  note("staff", "a hull is not set past its format", overCap.status >= 400 && /seats 40/.test(said(overCap)), said(overCap).slice(0, 100));
  const lateDrop = await stf.post("episodes", {
    slug: `e2e-late-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 4, price_cents: 0,
  ...
  const cmk = await stf.post("episodes", {
    slug: `e2e-comp-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 4, price_cents: 0,
  });
  const compVid = cmk.data?.[0]?.id;
  await stf.post("episode_segment_caps", [
    { episode_id: compVid, segment: "single_woman", cap: 1 },
    { episode_id: compVid, segment: "single_man", cap: 1 },
  ]);
  const compWait = await reg.post("passes", { episode_id: compVid, profile_id: uid(p.regional), status: "waitlist" });
  ...
  const compDel = await stf.del(`episodes?id=eq.${compVid}`);
  ...
  const hmk = await stf.post("episodes", {
    slug: `e2e-whole-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 4, price_cents: 1000,
    deposit_required: true, deposit_cents: 7000, status: "scheduled",
  });
  const hvid = hmk.data?.[0]?.id;
  const giver = await nat.post("passes", { episode_id: hvid, profile_id: uid(p.national), status: "aboard" });
  const giverRsvp = giver.data?.[0]?.id;
  const bed = await nat.rpc("claim_a_daybed", { p_pass: giverRsvp });
  note("national", "holds a daybed on the pass to hand on", bed.status < 300, `got ${bed.status} ${said(bed).slice(0, 80)}`);
  const bedTwice = await nat.rpc("claim_a_daybed", { p_pass: giverRsvp });
  ...
  const bedNow = await glo.get(`episode_daybeds?rsvp_id=eq.${giverRsvp}&select=profile_id`);
  note("global", "the daybed followed the pass", bedNow.data?.[0]?.profile_id === uid(p.global), JSON.stringify(bedNow.data).slice(0, 80));
  const strike = await stf.del(`episode_daybeds?rsvp_id=eq.${giverRsvp}`);
  ...
  const hdel = await stf.del(`episodes?id=eq.${hvid}`);
  ...
  const nmk = await stf.post("episodes", {
    slug: `e2e-near-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: new Date(Date.now() + 6 * 3600e3).toISOString(), time_zone: "America/New_York",
    passes_total: 4, price_cents: 1000, status: "scheduled",
  });
  const nvid = nmk.data?.[0]?.id;
  await stf.post("passes", { episode_id: nvid, profile_id: uid(p.global), status: "aboard" });
  const nearDel = await stf.del(`episodes?id=eq.${nvid}`);
  ...
  await stf.patch(`episodes?id=eq.${nvid}`, { status: "cancelled" });
  const nearDel2 = await stf.del(`episodes?id=eq.${nvid}`);
  ...
  const fmk = await stf.post("episodes", {
    slug: `e2e-level-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 6, price_cents: 0, status: "scheduled",
  });
  const fvid = fmk.data?.[0]?.id;
  await stf.post("episode_vessels", [{ episode_id: fvid, vessel_id: h1.data?.[0]?.id, position: 1 }, { episode_id: fvid, vessel_id: h2.data?.[0]?.id, position: 2 }]);
  for (const who of ["regional", "national", "global"]) await stf.post("passes", { episode_id: fvid, profile_id: uid(p[who]), status: "aboard" });
  const level = await stf.rpc("assign_vessels_evenly", { p_episode: fvid });
  const hulls = await stf.get(`passes?episode_id=eq.${fvid}&select=vessel_id`);
  ...
  const logged = await stf.get(`audit_log?table_name=eq.episodes&row_id=eq.${fvid}&action=eq.INSERT&select=actor_id`);
  ...
  const ref = await stf.get(`knots_ledger?episode_id=eq.${fvid}&select=member_ref&limit=1`);
  ...
  const fdel = await stf.del(`episodes?id=eq.${fvid}`);
  ...
  const prMk = await reg.post("member_event_proposals", { proposer_id: uid(p.regional), title: `E2E once ${stamp}`, series: "mixer" });
  const prId = prMk.data?.[0]?.id;
  ...
  const rq = await reg.post("charter_requests", { profile_id: uid(p.regional), series: "private_charter", party_size: 12, note: "E2E" });
  note("regional", "an on-request format has a door", rq.status === 201, `got ${rq.status} ${JSON.stringify(rq.data).slice(0, 80)}`);
  const rqAnon = await anon.post("charter_requests", { profile_id: uid(p.regional), series: "private_charter" });
...
async function decisionRules(p) {
  ...
  const fmk = await stf.post("episodes", {
    slug: `e2e-tentpole-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 60, price_cents: 0, hull_ceiling_heads: 60,
    hull_certificate: "E2E Tentpole · USCG COI · 60 heads",
  });
  const tentVid = fmk.data?.[0]?.id;
  const bigCaps = await stf.post("episode_segment_caps", [
    { episode_id: tentVid, segment: "single_woman", cap: 25 },
    { episode_id: tentVid, segment: "single_man", cap: 25 },
  ]);
  ...
  const overTent = await stf.post("episode_segment_caps", { episode_id: tentVid, segment: "couple", cap: 10 });
  ...
  const tentDel = await stf.del(`episodes?id=eq.${tentVid}`);
  note("staff", "the tentpole fixture is struck", tentDel.status < 300, `got ${tentDel.status}`);

  // — a sailing keeps its taxonomy —
  const tmk = await stf.post("episodes", {
    /* setting is deliberately NOT stated: the series fills it. Stating it and
       expecting the series to override was the old contract, retired when the
       Miami programme showed one series running afloat and ashore. */
    slug: `e2e-taxo-${stamp}`, title: "E2E fixture episode.", series: "pool_social",
    starts_at: plus30, ends_at: new Date(Date.parse(plus30) + 10 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 4, price_cents: 0,
  });
  note("staff", "an unstated setting is filled from the series, and ten hours is an odyssey",
    tmk.data?.[0]?.setting === "shore" && tmk.data?.[0]?.kind === "port_day" && tmk.data?.[0]?.sub_class === "odyssey",
    JSON.stringify({ c: tmk.data?.[0]?.setting, k: tmk.data?.[0]?.kind, s: tmk.data?.[0]?.sub_class }));
  await stf.del(`episodes?id=eq.${tmk.data?.[0]?.id}`);

  /* The other half of the same contract: a series DEFAULTS the axes, it does
     not dictate them. Off Soundings is an airboat one month and a polo field
     the next, so an episode that states its own setting keeps it. */
  const tmk2 = await stf.post("episodes", {
    slug: `e2e-taxo-own-${stamp}`, title: "E2E fixture episode.", series: "pool_social",
    setting: "sea", experience_class: "premium",
    starts_at: plus30, ends_at: new Date(Date.parse(plus30) + 3 * 3600e3).toISOString(),
    time_zone: "America/New_York", passes_total: 4, price_cents: 0,
  });
  note("staff", "an episode that states its own axes keeps them",
    tmk2.data?.[0]?.setting === "sea" && tmk2.data?.[0]?.experience_class === "premium",
    JSON.stringify({ c: tmk2.data?.[0]?.setting, x: tmk2.data?.[0]?.experience_class }));
  await stf.del(`episodes?id=eq.${tmk2.data?.[0]?.id}`);

  // — a couple names its second head; a guest still rides Global alone —
  const cmk = await stf.post("episodes", {
    slug: `e2e-couple-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 6, price_cents: 0,
  });
  const cplVid = cmk.data?.[0]?.id;
  const cplPass = await stf.post("passes", { episode_id: cplVid, profile_id: uid(p.regional), status: "aboard", segment: "couple" });
  const partner = await reg.post("pass_guests", { rsvp_id: cplPass.data?.[0]?.id, name: "E2E Partner", kind: "partner" });
  ...
  const partnerTwice = await reg.post("pass_guests", { rsvp_id: cplPass.data?.[0]?.id, name: "E2E Other", kind: "partner" });
  note("regional", "a couple has one second head", partnerTwice.status >= 400, `got ${partnerTwice.status}`);
  const soloPass = await stf.post("passes", { episode_id: cplVid, profile_id: uid(p.national), status: "aboard", segment: "single_man" });
  ...
  const amk = await stf.post("episodes", {
    slug: `e2e-heads-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 6, price_cents: 0,
  });
  const headVid = amk.data?.[0]?.id;
  const gloPass = await glo.post("passes", { episode_id: headVid, profile_id: uid(p.global), status: "aboard", guests: 2, guest_names: ["E2E One", "E2E Two"] });
  const shelf = await glo.get("addons?select=id,price_cents&active=eq.true&limit=1");
  const addonId = shelf.data?.[0]?.id;
  await glo.rpc("attach_addons", { p_pass: gloPass.data?.[0]?.id, p_addons: [addonId], p_qty: 3 });
  await glo.patch(`passes?id=eq.${gloPass.data?.[0]?.id}`, { guests: 1, guest_names: ["E2E One"] });
  const trimmed = await glo.get(`pass_addons?rsvp_id=eq.${gloPass.data?.[0]?.id}&select=qty`);
  ...
  const headDel = await stf.del(`episodes?id=eq.${headVid}`);
  ...
  const smk = await stf.post("episodes", {
    slug: `e2e-sponsored-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 6, price_cents: 1000,
  });
  const spVid = smk.data?.[0]?.id;
  const noAct = await stf.rpc("comp_a_pass_for_sponsor", { p_episode: spVid, p_sponsor: spMk.data?.[0]?.id, p_profile: uid(p.national) });
  ...
  await stf.post("episode_sponsors", { episode_id: spVid, sponsor_id: spMk.data?.[0]?.id });
  const comp = await stf.rpc("comp_a_pass_for_sponsor", { p_episode: spVid, p_sponsor: spMk.data?.[0]?.id, p_profile: uid(p.national) });
  const compRow = await stf.get(`passes?episode_id=eq.${spVid}&profile_id=eq.${uid(p.national)}&select=comp,sponsor_id`);
  const compCharge = await nat.get(`account_ledger?episode_id=eq.${spVid}&kind=eq.pass&select=delta_cents`);
  ...
  const memberComp = await nat.rpc("comp_a_pass_for_sponsor", { p_episode: spVid, p_sponsor: spMk.data?.[0]?.id, p_profile: uid(p.national) });
  ...
  const selfSponsor = await glo.post("passes", { episode_id: spVid, profile_id: uid(p.global), status: "waitlist", sponsor_id: spMk.data?.[0]?.id });
  ...
  await stf.del(`episodes?id=eq.${spVid}`);
  ...
  const dmk = await stf.post("episodes", {
    slug: `e2e-farewell-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 4, price_cents: 1000,
  });
  const fwVid = dmk.data?.[0]?.id;
  await stf.post("passes", { episode_id: fwVid, profile_id: uid(p.paused), status: "aboard" });
  const charged = await stf.get(`account_ledger?episode_id=eq.${fwVid}&profile_id=eq.${uid(p.paused)}&kind=eq.pass&select=delta_cents`);
  const leave = await pau.rpc("set_own_standing", { p_status: "departed" });
  const gone = await stf.get(`passes?episode_id=eq.${fwVid}&profile_id=eq.${uid(p.paused)}&select=status`);
  const squared = await stf.get(`account_ledger?episode_id=eq.${fwVid}&profile_id=eq.${uid(p.paused)}&kind=eq.credit&memo=like.Departed*&select=delta_cents`);
  ...
  const fwDel = await stf.del(`episodes?id=eq.${fwVid}`);
...
async function decisionsOfSept2(p) {
  ...
  const chart = await stf.get("cities?slug=in.(miami,los-angeles)&select=id,slug,name");
  ...
  const homes = await stf.get(`profiles?email=like.e2e-*@fixtures.invalid&select=email,home_city`);
  const homeOf = (who) => (homes.data ?? []).find((r) => r.email === `e2e-${who}@fixtures.invalid`)?.home_city ?? null;
  ...
  const amk = await stf.post("episodes", {
    slug: `e2e-away-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/Los_Angeles", passes_total: 6, price_cents: 0, status: "scheduled", city_id: la,
  });
  const awayVid = amk.data?.[0]?.id;
  note("staff", "an away sailing leaves from los angeles", !!awayVid && amk.data?.[0]?.city_id === la, `got ${amk.status} ${said(amk).slice(0, 90)}`);
  ...
  const toMiami = await stf.patch(`episodes?id=eq.${awayVid}`, { city_id: miami });
  const regHome = await reg.post("passes", { episode_id: awayVid, profile_id: uid(p.regional), status: "aboard" });
  ...
  const regRel = await reg.del(`passes?episode_id=eq.${awayVid}&profile_id=eq.${uid(p.regional)}`);
  note("regional", "and hands the home pass back", regRel.status < 300, `got ${regRel.status}`);

  const toOpen = await stf.patch(`episodes?id=eq.${awayVid}`, { city_id: null });
  const regOpen = await reg.post("passes", { episode_id: awayVid, profile_id: uid(p.regional), status: "aboard" });
  ...
  await reg.del(`passes?episode_id=eq.${awayVid}&profile_id=eq.${uid(p.regional)}`);
  await stf.patch(`episodes?id=eq.${awayVid}`, { city_id: miami });
  ...
  const unhomed = await stf.patch(`profiles?id=eq.${uid(p.regional)}`, { home_city: null });
  try {
    const noHome = await reg.post("passes", { episode_id: awayVid, profile_id: uid(p.regional), status: "aboard" });
    ...
    if (noHome.status === 201) await stf.del(`passes?id=eq.${noHome.data?.[0]?.id}`);
  } finally {
    const rehomed = await stf.patch(`profiles?id=eq.${uid(p.regional)}`, { home_city: miami });
    note("staff", "the regional fixture is homed in miami again",
      rehomed.status < 300 && rehomed.data?.[0]?.home_city === miami, `got ${rehomed.status} ${said(rehomed).slice(0, 80)}`);
  }
  const awayDel = await stf.del(`episodes?id=eq.${awayVid}`);
  ...
  const cmk = await stf.post("episodes", {
    slug: `e2e-cert-${stamp}`, title: "E2E fixture sailing.", setting: "sea", kind: "sea_day",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 4, price_cents: 0,
  });
  const certVid = cmk.data?.[0]?.id;
  const noCert = await stf.patch(`episodes?id=eq.${certVid}`, { hull_ceiling_heads: 41 });
  ...
  const shortCert = await stf.patch(`episodes?id=eq.${certVid}`, { hull_ceiling_heads: 41, hull_certificate: "ab" });
  note("staff", "a certificate is more than two letters", shortCert.status >= 400, `got ${shortCert.status} ${said(shortCert).slice(0, 90)}`);
  const withCert = await stf.patch(`episodes?id=eq.${certVid}`, { hull_ceiling_heads: 41, hull_certificate: "Trident 512 · USCG COI · 49 heads" });
  ...
  const below = await stf.patch(`episodes?id=eq.${certVid}`, { hull_ceiling_heads: 32, hull_certificate: null });
  ...
  const certBack = await stf.patch(`episodes?id=eq.${certVid}`, { hull_ceiling_heads: null, hull_certificate: null });
  const certDel = await stf.del(`episodes?id=eq.${certVid}`);
...
async function taxonomyOfSept2(p) {
  ...
  const cat = await stf.get("series?select=slug,category,experience_class,access,active&order=slug");
  ...
  /* Null is legal and load-bearing: it means the series runs both settings.
     Four of the five Season I series do. */
  const offSetting = live.filter((f) => f.category !== null && !SETTINGS.includes(f.category));
  note("staff", "and a category that says only where it happens, or nothing at all",
  ...
  const openBack = await stf.post("series", {
  ...
  try {
    ...
    if (openBack.status === 201) await stf.del(`series?slug=eq.${openSlug}`);
  }

  // — a sailing carries the class its format states —
  const unfiled = await stf.get("episodes?experience_class=is.null&select=id,slug&limit=5");
  ...
  const vmk = await stf.post("episodes", {
    slug: `e2e-xclass-${stamp}`, title: "E2E fixture sailing.", setting: "sea",
    starts_at: plus30, time_zone: "America/New_York", passes_total: 4, price_cents: 0,
    knots_multiplier: 0, distance_nm: 0,
  });
  ...
  try {
    ...
    /* Filing an episode whose class is already set does not move it — the
       series fills a null, it does not overwrite an answer. Clearing the class
       in the same write is what asks the series for one. */
    const filed = await stf.patch(`episodes?id=eq.${xVid}`, { series: target?.slug, experience_class: null });
    note("staff", "and the class is filled from the series when the episode leaves it blank",
    ...
    const trek = await stf.patch(`episodes?id=eq.${xVid}`, { sub_class: "trek" });
    ...
    const del = await stf.del(`episodes?id=eq.${xVid}`);
...
async function main() {
  ...
  /* THE SUITE'S LEDGER FOOTPRINT, pinned.
     ...
     dishonest to assert it: knots_ledger is append-only by policy, and one of
